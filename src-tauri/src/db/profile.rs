...
pub async fn get(pool: &PgPool) -> Result<Option<Profile>> {
    Ok(sqlx::query_as::<_, Profile>(
        "SELECT full_name, phone, email, links, interests FROM profile",
    )
    .fetch_optional(pool)
    .await?)
}
...
pub async fn upsert(pool: &PgPool, p: &Profile) -> Result<Profile> {
    Ok(sqlx::query_as::<_, Profile>(
        "INSERT INTO profile (id, full_name, phone, email, links, interests)
         ...
         ON CONFLICT (id) DO UPDATE
           SET full_name = EXCLUDED.full_name,
               ...
               updated_at = now()
         RETURNING full_name, phone, email, links, interests",
    )
    .bind(&p.full_name)
    .bind(&p.phone)
    .bind(&p.email)
